					...
					<table class="fc-list-table">
						...
						<tbody class="tbshowing">
							...
							<tr class="showingInput">
								<td><input type="text" id="title" class="showingInput"
									placeholder="공연명을 입력해주세요."></td>
								<td><input type="time" id="startTime" class="showingInput">
									~ <input type="time" id="endTime" class="showingInput"></td>
							</tr>
						</tbody>
					</table>
				</div>
...
<script>
	$(document).ready(function() {
		...
		function insertEvent() { //ajax 회차등록
			$('#myModal').modal('show');
			$('#title').val('');
			$('#startTime').val('');
			$('#endTime').val('');
			$('#btnInsert').on("click",	function() {
				...
				dayList.addEvent({
					title : $('#title').val(),
					start : moment($('#startTime').val(),
							'HH:mm').format(
							'YYYY-MM-DDTHH:mm:ss'),
					end : moment($('#endTime').val(), 'HH:mm')
